...
- Read the `clean-code-javascript` README and preserve the repository's simple documentation-focused structure.
- Keep examples readable and focused on clean-code principles without introducing unnecessary tooling.
- Preserve the repository's current project structure, naming conventions, coding style, validation commands, and branch intent.
